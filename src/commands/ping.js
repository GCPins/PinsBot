...
module.exports = class PingCommand extends SlashCommand {
  ...
  async run(ctx) {
    ...
    let symbol =
      ping < 0 ? ':exclamation:' : ping < 100 ? ':green_square:' : ping < 200 ? ':orange_square:' : ':red_square:';
    ...
    symbol =
      ping <= 0 ? ':exclamation:' : ping < 500 ? ':green_square:' : ping < 750 ? ':orange_square:' : ':red_square:';
    msg += `\n${symbol} Bot Latency is **${ping}**ms ${symbol}`;
    total += ping;

    ping = prev.timestamp - ctx.invokedAt;
    symbol =
      ping <= 0 ? ':exclamation:' : ping < 200 ? ':green_square:' : ping < 450 ? ':orange_square:' : ':red_square:';
    ...
    symbol =
      total <= 0 ? ':exclamation:' : total < 700 ? ':green_square:' : total < 900 ? ':orange_square:' : ':red_square:';
