...
module.exports = class MoveallCommand extends SlashCommand {
  ...
  async run(ctx) {
    ...
    if (!ctx.options.to && !ctx.options.from) {
      ...
      if (!tochl) return ctx.send(`You are not in a voice channel. Please use the full command or join a voice channel.`, { ephemeral: true });
      ...
      return ctx.send(`Moved **${numU}** user(s) from ${chlstr.slice(0, -1).slice(0,-1)} to ${tochl} (ID: \`${tochl.id}\`).`, { ephemeral: true });
    ...
    if (!ctx.options.from) {
      ...
      if (!fromchl) return ctx.send(`You are not in a voice channel. Please use the full command or join a voice channel.`, { ephemeral: true });
    ...
    if (!ctx.options.to) {
      ...
      if (!tochl) return ctx.send("You are not in a voice channel. Please user the full command or join a voice channel.", { ephemeral: true });
